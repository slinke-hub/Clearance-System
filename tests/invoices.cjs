...
test('US currency aliases work in columns, price headings, review, storage and export', () => {
  ...
  const exported = XLSX.read(generateEnrichedExcel([{ ...saved[0], currency: 'US$' }], 'test.xlsx'));
  assert.equal(XLSX.utils.sheet_to_json(exported.Sheets['ZATCA Classification'])[0].Currency, 'USD');
});
...
test('manual mapping and edits reach every classified item and export; ownership is enforced', async () => {
  const input = workbook([['Code', 'Details', 'Qty', 'Unit Price', 'Amount', 'Currency'], ...Array.from({ length: 8 }, (_, i) => ['P' + i, 'Lamp ' + i, 2, 3, 6, 'USD'])]);
  ...
  assert.equal(received[7].itemDescription, 'Reviewed OEM reference');
  ...
  const rows = XLSX.utils.sheet_to_json(book.Sheets['ZATCA Classification']);
  ...
  assert.equal(rows[7]['CUSTOMS DUTY FEES'], '5%');
...
test('provided CN07-210 invoice reconciles all 89 items', { skip: !sample }, () => {
  ...
  const output = generateEnrichedExcel(items.map(i => ({ row_index: i.rowIndex, item_name: i.itemName, item_description: i.itemDescription, item_code: i.itemCode, unit: i.unit, contract: i.contract, quantity: i.quantity, unit_price: i.unitPrice, total_price: i.totalPrice, currency: i.currency })), 'sample.xlsx', parsed.metadata);
  const book = XLSX.read(output);
  assert.equal(XLSX.utils.sheet_to_json(book.Sheets['ZATCA Classification']).length, 89);
